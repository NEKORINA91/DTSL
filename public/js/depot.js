...
async function saveStaff(){
  ...
  const res=await apiForm('/api/depot/staff',fd);
...
async function delStaff(id){
  ...
  await api('/api/depot/staff/'+id,'DELETE'); loadStaff();
}

// ══ BUSES ════════════════════════════════════════════
async function loadBuses(){
  [allBuses,allRoutes]=await Promise.all([api('/api/depot/buses'),api('/api/depot/routes')]);
...
function renderBuses(data){
  q('buses-tbody').innerHTML=data.length?data.map(b=>`<tr>
    ...
    <td style="display:flex;gap:.35rem;flex-wrap:wrap;">
      ${b.status==='active'?`<button class="btn btn-sm" style="background:#f59e0b;color:#fff;border:none;" onclick="setBusStatus(${b.id},'maintenance')">🔧 Maintenance</button>`:''}
      ${b.status==='maintenance'?`<button class="btn btn-sm" style="background:#16a34a;color:#fff;border:none;" onclick="setBusStatus(${b.id},'active')">✅ Set Active</button>`:''}
      ${b.status!=='retired'?`<button class="btn btn-red btn-sm" onclick="setBusStatus(${b.id},'retired')">Retire</button>`:''}
    </td></tr>`).join('')
...
async function saveBus(){
  ...
  if(id)await api('/api/depot/buses/'+id,'PATCH',body);
  else await api('/api/depot/buses','POST',body);
...
async function saveRoute(){
  ...
  if(id)await api('/api/depot/routes/'+id,'PATCH',body);
  else await api('/api/depot/routes','POST',body);
...
async function delRoute(id){
  ...
  await api('/api/depot/routes/'+id,'DELETE'); loadRoutes();
}

// ══ SCHEDULES (with weekly/monthly filter) ════════════
async function loadScheds(){
  if(!allBuses.length)allBuses=await api('/api/depot/buses');
  if(!allStaff.length)allStaff=await api('/api/depot/staff');
  if(!allRoutes.length)allRoutes=await api('/api/depot/routes');
  q('sc-route').innerHTML='<option value="">Select Route</option>'+allRoutes.map(r=>`<option value="${r.id}">${r.name}</option>`).join('');
  q('sc-bus').innerHTML='<option value="">Select Bus</option>'
    +allBuses.map(b=>{
      const disabled=b.status!=='active';
      const label=b.reg_number+(disabled?' ['+b.status.toUpperCase()+']':'');
      return `<option value="${b.id}" ${disabled?'disabled style="color:#9ca3af"':''}>${label}</option>`;
    }).join('');
...
async function refreshSchedView(){
  ...
  const url=schedPeriod==='all'?'/api/depot/schedules':`/api/admin/schedules?view=${schedPeriod}&date=${today}`;
...
async function loadLive(){
  ...
  allLive=await api('/api/depot/live');
  filterLive();
...
async function loadMaint(){
  ...
  allMaint=await api('/api/depot/maintenance');
  renderMaint(allMaint);

  // service due alerts
  const due=await api('/api/depot/maintenance/due');
  if(due.length){
...
async function saveMaint(){
  await api('/api/depot/maintenance','POST',{bus_id:q('mm-bus').value,service_date:q('mm-date').value,type:q('mm-type').value,description:q('mm-desc').value,cost:q('mm-cost').value,technician_name:q('mm-tech').value,next_service:q('mm-next').value||null});
...
async function loadReports(){
  ...
  const completion=await api('/api/depot/analytics/completion');
...
async function setBusStatus(id, status) {
  ...
  const msg = status==='maintenance'
    ? 'Mark this bus as under maintenance? All upcoming scheduled trips will be cancelled.'
    : status==='active'
      ? 'Mark this bus as active again?'
      : 'Retire this bus? This cannot be undone easily.';
  if (!confirm(msg)) return;
  const res = await api('/api/depot/buses/'+id+'/status','PATCH',{status});
  if (res.success) { loadBuses(); if (status==='maintenance') loadScheds(); }
